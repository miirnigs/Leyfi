...
void CServerDlg::InitCtrl()
{
	...
	// 往事件列表里添加成功信息
	SendMessage((WM_USER + 151), NULL, (LPARAM)"控件初始化成功");
}

void CServerDlg::InitSQL(string sqlPath)
{
	// 判断数据库是否存在
	if (_access(Converter::CombineString(Path::GetExePath(false), "Server.db").c_str(), 0) == -1)
	{
		// 数据库文件不存在
	...
	if (Sqlite::GetInstance().GetTables(lstTables))
	{
		...
		for (auto table : tables)
		{
			...
			{
				// 没找到其中的任意一个表
				if (MessageBoxA(Converter::CombineString("数据库表\"", table, "\"缺失！").c_str(), "错误", MB_ICONERROR | MB_OK) == IDOK)
				{
					// 退出程序
					PostQuitMessage(0);
				}
	...
	try
	{
		...
		for (auto column : column_3)
		{
			...
			{
				throw make_pair("Software", column);
			}
		}
		...
		for (auto column : column_5)
		{
			...
			{
				throw make_pair("RemoteData", column);
			}
		}
		...
		for (auto column : column_6)
		{
			...
			{
				throw make_pair("Setting", column);
			}
		}
	}
	catch (pair<const char*, char*> msg)		// 注意！！第一个模板类型是const char*，不小心填成char*编译器居然不会报错，但是运行会崩
	{
		// 没找到任何一个列
		if (MessageBoxA(Converter::CombineString("数据库表\"", msg.first, "\"", "的\"", msg.second, "\"列缺失！").c_str(), "错误", MB_ICONERROR | MB_OK) == IDOK)
		{
			// 退出程序
			PostQuitMessage(0);
		}
	...
	// 往事件列表里添加成功信息
	SendMessage((WM_USER + 151), NULL, (LPARAM)"数据库初始化成功");
}

void CServerDlg::LoadData()
{
	// 从数据库读取数据
	vector<vector<string> > lstData;		// 使用一个vector从数据库读取数据，不用clear或者swap
	...
	for (int i = 0; i < sizeof(tables) / sizeof(tables[0]); i++)
	{
		...
		{
			// 读取数据失败
			if (MessageBoxA(Converter::CombineString("从数据库表\"", tables[i], "\"读取数据失败！").c_str(), "错误", MB_ICONERROR | MB_OK) == IDOK)
			{
				// 退出程序
				PostQuitMessage(0);
			}
	...
	// 往事件列表里添加成功信息
	SendMessage((WM_USER + 151), NULL, (LPARAM)"数据库数据读取成功");
}
...
void CServerDlg::ShowDataCount()
{
	...
	m_statusbar.SetPaneText(1, Converter::CombineString("卡密数量: ", keyCount).c_str(), true);
	m_statusbar.SetPaneText(2, Converter::CombineString("软件数量: ", softwareCount).c_str(), true);
	m_statusbar.SetPaneText(3, Converter::CombineString("封禁数量: ", blockCount).c_str(), true);
}
...
BEGIN_MESSAGE_MAP(CServerDlg, CDialogEx)
	...
	ON_MESSAGE((WM_USER + 150), OnListControlChanged)
	ON_MESSAGE((WM_USER + 151), OnListControlAdd)
...
void CServerDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	case 1:		// 定时器id为1的定时器
	{
		...
		m_statusbar.SetPaneText(5, Converter::CombineString("当前时间: ",
			time.tm_year + 1900, "年",
			time.tm_mon + 1, "月",
			time.tm_mday, "日 ",
			time.tm_hour, ":",
...
LRESULT CServerDlg::OnListControlAdd(WPARAM wParam, LPARAM lParam)
{
	...
	m_listevent.InsertItem(nIndex, Converter::CombineString(
		time.tm_year + 1900, "年",
		time.tm_mon + 1, "月",
		time.tm_mday, "日 ",
		time.tm_hour, ":",
